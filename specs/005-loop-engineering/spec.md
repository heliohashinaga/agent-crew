...
- Q4 (deterministic gate checks): **B — Pluggable deterministic check set.** The `loop_engine` core defines a **pluggable** deterministic-check list and ships a default set (artifact referenced exists + any suite/contract check supplied by the caller or the concrete actor binding); specific checks are supplied by callers/bindings so the core stays generic and network-free. The core must never hard-code binding-specific checks (e.g. a media-loop must not assume an auth `lint` check).
- Q5 (stall status): **A — `stalled` is a distinct final status.** A loop that stops making progress (ratchet) reports `status == "stalled"` (distinct from `exhausted`), still a non-zero exit in the same family as `exhausted`. This gives operators an actionable "ran but stopped progressing" signal without weakening the termination guarantee.
- Q6 (actor exception): **A — budget-bounded retry, separate from `max_iterations`.** When the actor raises an unexpected exception (not a gate failure), the iteration is recorded as **failed** and repaired via the repair path, but this **does not consume a `max_iterations` slot**; retries are bounded by the **budget** (time/tokens/cost). If budget is exhausted, escalate (FR-004). This keeps unexpected crash-loops from burning the real gate-failure iteration budget.
- Q7 (budget semantics): **A — hard stop in `loop_engine`** (an intentional, scoped divergence from the parent factory's soft-budget convention). Within `loop_engine`, exceeding a configured budget dimension is a **hard termination → `exhausted`** (FR-003, SC-002). The parent ai-factory's soft-budget (warn + continue) applies to its own cost tracking, not to loop termination; the loop layer hard-stops because unbounded autonomous loops are the failure mode loop engineering exists to prevent. (A future `budget_soft` opt-in flag is a possible extension, not v1.)
...
- What happens when the actor fails with an exception (not just a gate failure)? Per **Q6=A**: the iteration is recorded as **failed**, repaired via the repair path, and **does not consume a `max_iterations` slot**; retries are bounded by the **budget**, escalating when budget is exhausted — never a silent crash of the whole loop unless configured to fail fast.
- What happens when the budget is exhausted? Per **Q7=A**: **hard stop → `exhausted`** (escalation). This is an intentional divergence from the parent factory's soft-budget (warn + continue) convention, scoped to `loop_engine`; unbounded autonomous loops are the failure mode this feature prevents.
...
- **FR-003**: The loop MUST support configurable **termination conditions**: `max_iterations` (required), an optional **budget** (time/tokens/cost), and an optional **progress/stall ratchet**. Termination MUST be guaranteed: with a never-passing gate, the loop performs at most `max_iterations` iterations. A no-progress (ratchet) termination reports a **distinct `stalled` status** (Q5=A), not `exhausted`; both are non-pass outcomes (FR-004 escalation applies to both).
...
- **FR-011**: Per Q2=C, the gate MUST support two stages: (a) **deterministic checks** — network-free, MUST pass first; (b) an **independent reviewer** gate (a separate role/model) that runs only after (a) passes. The deterministic-check set is **pluggable** (Q4=B): the core ships defaults (artifact referenced exists + any suite/contract check supplied by the caller or binding) and never hard-codes binding-specific checks. Stage (b) MUST be integration-gated (`-m integration`); the deterministic core MUST remain network-free; when the network/LLM is unavailable, stage (b) surfaces cleanly (typed error or documented skip) — never a silent pass.
