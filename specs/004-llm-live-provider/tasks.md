...
- [x] **T002 — Scaffold `openai_compatible` package [Red→Green]** — Add
...
- [x] **T003 — `OpenAICompatibleProvider` registered & buildable [Red→Green]** —
...
- [x] **T004 — Resolve creds from SecretSource/env, never commit [Red→Green]** —
...
- [x] **T005 — `complete()` posts `/v1/chat/completions` & parses `LLMResult`
...
- [x] **T006 — Per-call model/temperature/max_tokens overrides [Red→Green]** —
...
- [x] **T007 — Error typing + secret redaction [Red→Green]** — Non-2xx status and
...
- [x] **T010 — `model_map.py` with defaults + JSON + env override [Red→Green]** — Add
...
- [x] **T020 — `runner.py` dual-mode switch [Red→Green]** — Add
...
- [x] **T021 — Opt-in gate: creds alone never go live [Red→Green]** — A run goes
      ...
      (opt-in + creds) → live.
- [x] **T022 — Fail-closed on unresolvable model id [Red→Green]** — If a live role's
...
- [x] **T023 — Wire dual-mode into the graph [Red→Green]** — Update
...
- [x] **T024 — Wire the shared provider into the `researcher` web scope
...
- [x] **T030 — Offline suite runs with network blocked [Red→Green]** — A test
...
- [x] **T031 — Telemetry invariants [Green]** — Live-mode emissions record role,
      capability level, resolved model id, tokens, cost, latency; no API key in
      any telemetry/log (reuse redaction, FR-018/SC-005).
- [x] **T032 — Whole-suite green gate (offline) [Green]** — `uv run ruff check .`
...
- [x] **T040 — Live integration path [Red→Green]** — Under `-m integration`, run
...
- [x] **T041 — Docs & quickstart [Green]** — Document the env-vars config surface
...
Pass criteria:
- [x] `create_provider("openai-compatible")` returns a working stdlib provider;
      unknown names still raise (SC-001).
- [x] Default `dev_workflow` (no opt-in) is **offline**: unit+contract suite
      passes with the network blocked and no HTTP call outside `-m integration`
      (SC-002, SC-006).
- [x] Live mode (`AI_FACTORY_LIVE=1` + creds + stubbed transport, or the
...
- [x] Configuring models is env-var-only (opencode-go vs openrouter by base_url/
      model/api_key); no committed config (SC-003).
- [x] No secret is emitted in any error/log/telemetry (SC-005).
- [x] `uv run ruff check .` and `uv run pytest` (unit+contract, network-free) pass;
      `-m integration` is best-effort gated (SC-004).
- [x] The `researcher` web scope can consume the shared `openai-compatible` provider
