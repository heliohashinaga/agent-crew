"""code-worker library CLI (T049, FR-011)."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="code-worker", description="Implement a technical plan."
    )
    parser.add_argument(
...
def main(argv: list[str] | None = None) -> int:
    ...
    product = implement(plan, Path(args.repo))
    write_stdout(emit(product, args.format))
    record_dev_invocation(
        "code_worker",
        args.run_id or "code-worker-auto",
        args.telemetry,
        result="pass" if product.validation == "passed" else "fail",
    )
    return 0
